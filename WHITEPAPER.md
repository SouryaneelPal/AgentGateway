...
§5.4 lists what is deliberately absent. This section says what closing each gap would actually take, because "future work" is only an honest label if you can name the work.

**What has been validated, and at what scale.** Four measurements exist, and it is worth being precise that they are laptop-scale rather than production-scale. `docs/chaos-report.md` runs four failure scenarios live — replayed mandate, concurrent spend-cap contention, tampered webhook signature, duplicate webhook delivery — each verified against the audit trail rather than asserted. `docs/load-profile.md` measures the end-to-end path with a real Razorpay test-mode call in every request; `docs/load-profile-gateway-only.md` repeats it with Razorpay stubbed, isolating the gateway's own cost at roughly a tenth of the end-to-end figure. `docs/soak-test.md` then holds a moderate load steady for sixteen minutes: 23,708 requests at roughly 25 req/s with **zero errors**, resident memory oscillating around 190 MB with no upward trend, Postgres connections bounded at one or two active and demonstrably returned to the pool, and p50 flat at 32 ms from the first sample to the last. The hostile-input and error-shape work behind those numbers is recorded in `HARDENING.md`.

That is enough to establish that the guardrails hold under contention and that nothing drifts over a sustained run. It is **not** evidence about behaviour across regions, across process restarts under partial failure, or at a request rate an actual payment processor would see. The two items below are precisely the gap between those two statements.

**mTLS between services.** Today every hop inside the stack is plaintext on a Docker bridge network: the gateway speaks to Postgres and Redis unencrypted, and merchant-facing traffic is authenticated by an API key over ordinary TLS. Closing this is less about the transport settings than about certificate lifecycle. The transport half is genuinely small — Postgres wants `ssl = on` with `hostssl … clientcert=verify-full` in `pg_hba.conf` and a `sslmode=verify-full` connection string carrying `sslcert`/`sslkey`/`sslrootcert`; Redis 6+ wants `tls-port`, `tls-cert-file`, `tls-key-file`, `tls-ca-cert-file` and `tls-auth-clients yes`, with matching `tls` options passed through ioredis. That is a day or two of configuration and a careful re-run of the test suite against TLS-enabled dependencies.

The real work is the PKI. A local CA generated once and copied around is not mTLS in any meaningful sense — it is a shared secret with extra steps, and it fails the moment a certificate expires at 3am. Production needs an issuing CA with a defined root and intermediate, short certificate lifetimes, automated renewal, and a revocation story: HashiCorp Vault's PKI secrets engine, AWS Private CA, or Smallstep's `step-ca`, with `cert-manager` driving issuance and rotation if the deployment is on Kubernetes. The lighter alternative is a service mesh — Linkerd in particular, whose identity derives from Kubernetes ServiceAccounts and which rotates workload certificates on roughly a 24-hour cycle with no application changes — but a mesh only secures mesh-internal traffic, so Postgres and Redis still need their own TLS unless they are brought in-mesh. Merchant-facing mTLS is a separate cost again: it obliges every merchant to hold and rotate a client certificate, which is an onboarding and support workflow, not a config flag. **Estimate: 2–3 days for transport configuration, 2–3 weeks to do the PKI, rotation and expiry runbooks properly.** The second number is the honest one.

**Multi-region webhook redundancy.** Today there is one gateway process, one registered webhook URL, and one Postgres. If that region is unreachable when Razorpay delivers, the system depends entirely on Razorpay's retry schedule to survive the outage. Making the *compute* multi-region is straightforward, because the gateway is stateless — replicas in two or more regions behind a health-checked front door, with Route 53 failover records or a Cloudflare load balancer resolving to the healthy one. Even that carries a recovery floor set by DNS TTL, so "redundant" means tens of seconds, not zero.

The state tier is where the effort actually goes. Two invariants this system rests on are both single-database facts. The spend cap in §3.5 is correct because of `SELECT … FOR UPDATE` on one row; the webhook redelivery guard in §3.4 is correct because `webhook_events.razorpay_event_id` is `UNIQUE` in one database. Put two regions in front of an asynchronously-replicated primary and both weaken: a failover with non-zero RPO can lose the row that proved an event was already processed, and concurrent delivery to two regions can settle the same payment twice. Preserving them means either a single writable primary with cross-region replicas and an accepted RPO plus a promotion runbook, or a distributed SQL engine that keeps the isolation semantics — CockroachDB, Aurora Global Database, or Spanner — which is a migration and a re-validation of §3.5's concurrency test, not a configuration change. One piece of this is already in place by design: §3.2's replay guard treats Redis as a fast path and the Postgres unique constraint as the durable authority, so regional Redis instances are acceptable without an active-active store. **Estimate: 3–4 weeks, dominated by the data tier and by actually rehearsing failover rather than by writing code**, plus a standing multi-region infrastructure cost that a test-mode buildathon entry has no reason to carry.

**Why neither was built now.** Both are deployment-topology problems whose value is only demonstrable in a deployment topology this project does not have. Adding a local CA and a second Docker container would produce something that *looked* like mTLS and multi-region without providing either property, which is precisely the kind of overclaiming §5.4 exists to avoid. The adapter, policy and trust-boundary work that the buildathon is actually judging on is unaffected by either.
